...
| `ValidityStartDate` | `EKKO` | `KDATB` | `Date` |  | Validity Per. Start |  |  |  |
| `ValidityEndDate` | `EKKO` | `KDATE` | `Date` |  | Validity Period End |  |  |  |
...
| `Quantity` | `EKKN` | `MENGE` | `Decimal(13,3)` |  | Quantity | PurchaseOrderQuantityUnit |  |  |
...
| `NetPriceQuantity` | `EKPO` | `BPRME` | `Decimal(5,0)` |  | Price Unit | OrderPriceUnit |  |  |
...
| `AccountAssignmentCategory` | `EKPO` | `KNTTP` | `String(1)` |  | Acct Assignment Cat. |  |  |  |
| `GoodsReceiptIsNonValuated` | `EKPO` | `WEUNB` | `Boolean` |  | GR Non-Valuated |  |  |  |
